...
namespace HotelProject.WebUI.Controllers
{
    public class AdminRoomController : Controller
    {
        ...
        public async Task<IActionResult> AddRoom(CreateRoomDto createRoomDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:63512/api/Room", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:63512/api/Room/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:63512/api/Room/{id}");
        ...
        public async Task<IActionResult> UpdateRoom(UpdateRoomDto updateRoomDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync($"http://localhost:63512/api/Room/", stringContent);
